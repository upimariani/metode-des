		...
		<div class="col-lg-12 d-flex align-items-stretch">
			<div class="card w-100">
				<div class="card-body p-4">
					<h5 class="card-title fw-semibold mb-4">Informasi Hasil Analisis Per Umur <?= $umur_bawah ?> - <?= $umur_atas ?> tahun</h5>
							...
							<div class="card-body">
								<div class="d-sm-flex d-block align-items-center justify-content-between mb-9">
									<div class="mb-3 mb-sm-0">
										<h5 class="card-title fw-semibold">Grafik Peramalan Penyakit Per Umur</h5>
									</div>

								</div>
								<canvas id="pengidap_umur" height="150"></canvas>
							</div>
						</div>
					</div>
					<div class="card">
	...
	<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form action="<?= base_url('RekamMedis/cAnalisisPerUmur/hitung') ?>" method="POST">
					<div class="modal-body">
						<div class="mb-3">
							<input type="hidden" name="umur_bawah" value="<?= $umur_bawah ?>">
							<input type="hidden" name="umur_atas" value="<?= $umur_atas ?>">
							...
							<select id="pengidap" name="penyakit" class="form-control">
								...
								?>
									...
									<?php
									//cek analisis
									$data = $this->db->query("SELECT * FROM `analisis_umur` WHERE thn_periode='" . $value->tahun . "' AND id_penyakit ='" . $value->id_penyakit . "' AND nama_desa='" . $value->alamat . "' AND umur_bawah='" . $umur_bawah . "' AND umur_atas='" . $umur_atas . "'")->row();
									if (!$data) {
										if ($urutan++ == '1') {
									?>
