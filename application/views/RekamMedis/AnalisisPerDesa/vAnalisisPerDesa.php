							...
							<div class="card-body">
								<div class="d-sm-flex d-block align-items-center justify-content-between mb-9">
									<div class="mb-3 mb-sm-0">
										<h5 class="card-title fw-semibold">Grafik Peramalan Penyakit</h5>
									</div>

								</div>
								<canvas id="jml_pengidap" height="150"></canvas>
							</div>
						</div>
					</div>
					<div class="card">
	...
	<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form action="<?= base_url('RekamMedis/cAnalisisPerDesa/hitung') ?>" method="POST">
						...
						<div class="mb-3">
							...
							<select id="pengidap" name="penyakit" class="form-control">
								...
								foreach ($periode as $key => $value) {
									//cek analisis
									$data = $this->db->query("SELECT * FROM `analisis_perdesa` WHERE thn_periode='" . $value->tahun . "' AND id_penyakit ='" . $value->id_penyakit . "' AND nama_desa='" . $value->alamat . "'")->row();
									if (!$data) {
										if ($urutan++ == '1') {
								?>
