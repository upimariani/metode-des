...
class cLaporanJK extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('KepalaPuskesmas/vLaporanJk');
	...
	public function cetak()
	{
		...
		$pdf->Cell(200, 40, 'LAPORAN HASIL PERAMALAN PER JENIS KELAMIN', 0, 0, 'C');
		$pdf->SetLineWidth(0);
		$pdf->Cell(10, 30, '', 0, 1);
		...
		$data = $this->mLaporan->laporan_jk($id_penyakit, $desa, $jk);
		foreach ($data as $key => $value) {
			$pdf->Cell(10, 6, $no++, 1, 0, 'C');
			$pdf->Cell(30, 6, $value->nama_penyakit . ' / ' . $value->jk, 1, 0);
