...
class cLaporanUmur extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('KepalaPuskesmas/vLaporanUmur');
	...
	public function cetak()
	{
		...
		$pdf->Cell(200, 40, 'LAPORAN HASIL PERAMALAN PER UMUR', 0, 0, 'C');
		$pdf->SetLineWidth(0);
		$pdf->Cell(10, 30, '', 0, 1);
		...
		$umur = $this->input->post('thn');
		$data = $this->mLaporan->laporan_umur($id_penyakit, $desa, $umur);
		foreach ($data as $key => $value) {
			$pdf->Cell(10, 6, $no++, 1, 0, 'C');
			$pdf->Cell(30, 6, $value->nama_penyakit . ' / ' . $value->umur_bawah, 1, 0);
			$pdf->Cell(30, 6, $value->thn_periode, 1, 0);
